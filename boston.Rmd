...
This project uses a public dataset named "Boston" from the R package - "MASS". This dataset studies the effects of a range of variables on median house prices in Boston in late 70s, United States. 

I will statistically analyse the dataset with machine learning algorithms. I will find out the effects of each variables on the median house prices and build a model that can make predictions of the house prices correctly.
...
* There is no missing data (either written or "NA") in the dataset either in column or row. 
...
* All variables seem to have different distributions.    
...
* Outliers exists in many variables include *black, crim, dis, lstat, medv, ptratio, zn,* and even in the Gaussian distribtued *rm*。
...
This section is meant to select the variables that is related to the topic of this project and eliminated irrelevant ones. 

However, since the aim of this project is to analyse each and every single variables in the dataset and their relatability to the median house prices in Boston, represented by "medv", I will therefore keep all variables. 
...
* In term of impact on median house prices, variables that have **positive** significant relationship with median house prices are "rm", "chas", "rad", "zn", and "black".
...
* In term of impact on median house prices, variables that have **negative** significant relationship with median house prices are "nox", "dis", "ptratio", "lstat", "crim", and "tax". 
...
* The most influential variable on median house prices in a positive way is the number of room a house has.

* The most influential variable on median house prices in a negative way is the concentration of nitrogen oxides in the air.

* The proportion of non-retail business acrs per town "indus" and the proportion of owner-occupied units built prior to 1940 "age" do not have significant impact on median house prices. 



### 5.4 Multicollinearity Detection

There is a high correlation between "rad" and "tax". Therefore, one of them will be kept in the future prediction model. This is a multicollinearity problem that the its existence would affect the standard errors of our coeffcients, widdening the range of 95% confidence interval, and making p-values invalid. 

This is identifiable via correlogram of the predictors in the dataset. As a rule of thumb, a correlation of between 0.8 to 1 indicates multicollinearity issue. 
...
An alternative popular test is called variance inflation factor (VIF). A VIF that exceed 5 indicates a problematic amount of collinearity (James et al. 2014).
...
This section describe whether the assumptions of linear regression is fulfilled.

* The relationship between independent variables and the responding variable is non-linear. It can be assessed through the relationship  graph in section 4.3 and following model diagnostic graph.   
...
```
* There is also no multvariate normality shown by a standard Q-Q plot below formed by the multiple linear regression model. An ideal trend is when all points are falling near the straight line in the plot. 
...
Thanks to diagnostic results that reveal non-linear model shoud be used.

However, I will use a lasso regression in this section which is a model with linear assumption. I would like to use it as a baseline model to compare with other models built in later section. 

Generally, lasso regression is selected when there is multicollinearity issue within the dataset and lasso can help to select 1 variable from the highly correlated group. A drawback is that the removal of correlated group actually depends on the selection of lambda and a trade-off would occur between the removal of  with the model performance metric.  
...
Identified best lambda is 0.0142.
...
The R2 is 0.76, meaning that the observed and predicted outcome values has a good correlation. The prediction error RMSE is 4.587 which representing an eror rate of 20.27%, which is considered high enough that an alternative model should be explored.  
...
data.frame(
  R2_Value_lasso = R2(predictions_lasso, test.data$medv),
  RMSE_Value_lasso = RMSE(predictions_lasso, test.data$medv)) %>% 
  mutate(Error_rate_lasso = RMSE_Value_lasso/mean(test.data$medv) *100)
...
Applying elastic net, which is a relative of lasso that incorportate the effect of ridge regression in its algorithm.
...
R shows that the best lambda is 0.0895.
...
This model has a similar R2, RMSE, and error rate as the lasso regression. 
...
data.frame(
  R2_value_Elas = R2(predictions_elas, test.data$medv),
  RMSE_value_Elas = RMSE(predictions_elas, test.data$medv)) %>% 
  mutate(Error_rate_Elas = RMSE_value_Elas/mean(test.data$medv) * 100)
...
KNN is the first non-parametric algorithm that can be used for dataset that cannot fulfill the linearity assumptions of regression, additionally, this method is immune to multicollinearity.
...
KNN has the higher error rate during prediction using the test data. It indicates that this algoritm is not suitable for this dataset. 
...
# Model performance
data.frame(
  R2_value_knn = R2(predictions_knn, test.data$medv),
  RMSE_Value_knn = RMSE(predictions_knn, test.data$medv)) %>% 
  mutate(Error_Rate_knn = RMSE_Value_knn/mean(test.data$medv) * 100)
...
It is expecting that decision tree should perform better than linear regression in this dataset when the there is highly non-linear relationship and this method is also immune to multicolliearity issue. 
...
As shown by following plot. 

```{r}
plot(model_DT)

```
The performance metric of decision tree show that 
...
# Model performance
data.frame(
  R2_Value_DT = R2(predictions_DT, test.data$medv),
  RMSE_Value_DT = RMSE(predictions_DT, test.data$medv)
) %>% 
  mutate(Error_Rate_DT = RMSE_Value_DT/mean(test.data$medv) * 100)
